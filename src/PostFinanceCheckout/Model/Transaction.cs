...
namespace PostFinanceCheckout.Model
{
    ...
    public partial class Transaction : IValidatableObject
    {
        ...
        public Transaction(Transaction parent = default(Transaction), PaymentConnectorConfiguration paymentConnectorConfiguration = default(PaymentConnectorConfiguration), TransactionState? state = default(TransactionState?), TransactionGroup group = default(TransactionGroup), TransactionCompletionBehavior? completionBehavior = default(TransactionCompletionBehavior?), ChargeAttemptEnvironment? authorizationEnvironment = default(ChargeAttemptEnvironment?), FailureReason failureReason = default(FailureReason), CustomersPresence? customersPresence = default(CustomersPresence?), TransactionEnvironmentSelectionStrategy? environmentSelectionStrategy = default(TransactionEnvironmentSelectionStrategy?), TokenizationMode? tokenizationMode = default(TokenizationMode?), TransactionUserInterfaceType? userInterfaceType = default(TransactionUserInterfaceType?), PaymentTerminal terminal = default(PaymentTerminal), Token token = default(Token), ModelEnvironment? varEnvironment = default(ModelEnvironment?), Address shippingAddress = default(Address), Address billingAddress = default(Address))
        {
            this.Parent = parent;
