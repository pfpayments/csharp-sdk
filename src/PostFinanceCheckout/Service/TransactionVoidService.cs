...
namespace PostFinanceCheckout.Service
{
    ...
    public partial class TransactionVoidService : ITransactionVoidService
    {
        ...
        /// <returns></returns>
        public TransactionVoidService(PostFinanceCheckout.Client.Configuration configuration = null)
        {
        ...
        public ApiResponse< List<TransactionVoid> > SearchWithHttpInfo (long? spaceId, EntityQuery query)
        {
            ...
            return new ApiResponse<List<TransactionVoid>>(localVarStatusCode,
                    localVarResponse.Headers
                        .GroupBy(x => x.Name, x => x.Value.ToString())
                        .ToDictionary(x => x.Key,  x => String.Join(", ", x)),
                    (List<TransactionVoid>) this.Configuration.ApiClient.Deserialize(localVarResponse, typeof(List<TransactionVoid>)));
        }
        /// <summary>
